...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir/*, float visibility*/) {
	...
	// Combine
	vec3 ambient = light.intensity * light.ambient * attenuation;
	vec3 diffuse = light.intensity * light.diffuse * diff * attenuation/* * visibility*/;
	vec3 specular = light.intensity * light.specular * spec * attenuation/* * visibility*/;
